...
from .cluster_utils import pairwise_distance, pairwise_distance_st
from .kmeans import kmedoids, batch_kmedoids, batch_kmedoids_st
from .spectral import batch_spectral_clustering
from .fast_kmeans import fast_kmedoids, batch_fast_kmedoids_with_split, batch_fast_kmedoids_with_split_st
...
def kmedoids_vs_fast_kmedoids_speed_test():
    ...
    # speed test
    iters = 200
    X = torch.rand(300, 1048)
    X = X.cuda(0)
    X = X.half()
    X_s = torch.rand(300, 1)
    X_s = X_s.cuda(0)
    #X_s = X_s.half()
    X_y = torch.rand(300, 1)
    X_y = X_y.cuda(0)
    #X_y = X_y.half()
    K = 49
    distance_matrix = pairwise_distance_st(X, X, X_s, X_s, X_y, X_y, metric='euclidean', all_negative=True)
...
def batch_kmedoids_speed_test():
    """speed test for batch_kmedoids_st() and batch_fast_kmedoids_with_split_st()"""
    # speed test
    K = 49
    X = data_generate(cluster=K)
    X_s = X.clone()  # Assuming X_s and X_t are clones of X for this example
    X_t = X.clone()
    X = X.cuda(0)
    X_s = X_s.cuda(0)
    X_s = X_s[..., 0].unsqueeze(-1)
    X_t = X_t.cuda(0)
    ...
    start = time.time()
    for i in range(10):
        assign, ids = batch_kmedoids_st(X, X_s, X_t, K, distance='euclidean', threshold=1e-5, iter_limit=60,
                                        id_sort=True, batch_distance=True, norm_p=1.0)
    torch.cuda.synchronize()
    end = time.time()

    print("batch kmedoids_st", end - start)
    print("max_mem", torch.cuda.max_memory_allocated())

    torch.cuda.synchronize()
    start = time.time()
    for i in range(10):
        assign_f, ids_f = batch_fast_kmedoids_with_split_st(X, X_s, X_t, K, distance='euclidean', threshold=1e-5, iter_limit=60,
                                                            id_sort=True, norm_p=1.0)

    torch.cuda.synchronize()
    end = time.time()

    print("batch fast_kmedoids_st", end - start)
    print("max_mem", torch.cuda.max_memory_allocated())
