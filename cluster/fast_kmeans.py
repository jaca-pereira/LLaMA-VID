...
"""
import torch
from .cluster_utils import pairwise_distance, KKZ_init, pairwise_distance_st
...
@torch.no_grad()
@torch.cuda.amp.custom_fwd(cast_inputs=torch.float32)
def batch_fast_kmedoids_with_split_st(X, X_s, X_t, K, distance='euclidean', threshold=1e-5, iter_limit=60,
                                    id_sort=True, norm_p=2.0, split_size=4, pre_norm=False):
    """
    split a batch tensor into multiple chunks in order to avoid OOM of GPU mem
    Args:
    ...
    if X.shape[0] > split_size:
        all_t = torch.split(X, split_size, dim=0)
        all_s = torch.split(X_s, split_size, dim=0)
        all_tt = torch.split(X_t, split_size, dim=0)
        assign_l, medoids_l = [], []
        for x_tmp, x_stmp, x_ttmp in zip(all_t, all_s, all_tt):
            assign, medoids = batch_fast_kmedoids_st(x_tmp, x_stmp, x_ttmp, K, distance=distance, threshold=threshold,
                                                    iter_limit=iter_limit,
                                                    id_sort=id_sort, norm_p=norm_p)
            assign_l.append(assign)
            medoids_l.append(medoids)
        ...
        assign, medoids = batch_fast_kmedoids_st(X, X_s, X_t, K, distance=distance, threshold=threshold,
                                                iter_limit=iter_limit,
                                                id_sort=id_sort, norm_p=norm_p)
        return assign, medoids
...
@torch.no_grad()
@torch.cuda.amp.custom_fwd(cast_inputs=torch.float32)
def batch_fast_kmedoids_st(X, X_s, X_t, K, distance='euclidean', threshold=1e-5, iter_limit=60,
                        id_sort=True, norm_p=2.0):
    """
    perform batch k mediods
    Args:
