...
class LLaMAVIDMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            text_size = cur_input_ids.shape[1]
            top_k = max(0, self.config.max_length - text_size)

            if images.ndim() == 4 and images.shape[0] == 1:
                kth = 1  # equivalent to doing nothing
            else:
                if images.ndim() == 4 and images.shape[1] < 96 or images.ndim()==5 and images[batch_idx].shape[1] < 96:
                    kth = 2
